...
class CalendarAdapter(context: Context, cal: Calendar) {
    ...
    var mHEventList = ArrayList<Event>()
    ...
    fun addEventList(eventList: ArrayList<Event>) {
        mEventList=eventList
    }
    fun addHEventList(eventList: ArrayList<Event>) {
        mHEventList=eventList
    }
    fun removeEvent(){
        mEventList.clear()
    }
    ...
    fun refresh() {
        ...
        for (i in offset until length + offset) {
            ...
            val imgHEventTag = view.findViewById<View>(R.id.img_event_holiday_tag) as ImageView
            ...
            for (j in mEventList.indices) {
                ...
                if (day.year == event.year
                    ...
                    imgEventTag.setColorFilter(Color.WHITE, PorterDuff.Mode.SRC_ATOP)
                }else{}
            }
            for (j in mHEventList.indices) {
                val event = mHEventList[j]
                if (day.year == event.year
                    && day.month == event.month
                    && day.day == event.day) {
                    imgHEventTag.visibility = View.VISIBLE
                    imgHEventTag.setColorFilter(Color.parseColor("#1E7FDF"), PorterDuff.Mode.SRC_ATOP)
                }
            }
